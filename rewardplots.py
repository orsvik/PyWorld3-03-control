...
hwi_sig_vals_xs = np.array([0.000, 0.550, 0.700, 0.800, 1.000])
...
ef_sig_vals_xs = np.array([0.0, 1.1, 4.0])
...
fig, (hwi_ax, hef_ax) = plt.subplots(1, 2, sharey=True, figsize=(6, 4))
fig.suptitle('Illustration of HWI and HEF rewards')
hwi_ax.plot(hwi_demo_xs, hwi_demo_ys, label="HWI reward")
hwi_ax.set_xlabel('HWI')
hwi_ax.set_ylabel('HWI reward')
hwi_ax.plot(hwi_sig_vals_xs, hwi_sig_vals_ys, '.', color='k')
hwi_ax.grid(True)
hef_ax.plot(ef_demo_xs, ef_demo_ys, label="HEF reward")
hef_ax.set_xlabel('HEF')
hef_ax.set_ylabel('HEF reward')
hef_ax.grid(True)
hef_ax.plot(ef_sig_vals_xs, ef_sig_vals_ys, '.', color='k')
plt.show()

for i in range(len(ef_sig_vals_xs)):
    print(ef_sig_vals_xs[i], ef_sig_vals_ys[i])